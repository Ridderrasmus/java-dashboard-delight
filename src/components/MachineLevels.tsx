  ...
  useEffect(() => {
    const fetchLevels = async () => {
      try {
        ...
        try {
          const waterData = await api.get<number>("/api/Machine/WaterLevel");
        ...
        try {
          const coffeeData = await api.get<number>("/api/Machine/CoffeeLevel");
      ...
      <section className="py-12 bg-black/20 backdrop-blur-sm">
        <div className="container mx-auto px-4">
          <div className="text-center">
            <p className="text-lg text-muted-foreground">Loading machine status...</p>
          </div>
        </div>
      </section>
    );
